...
UPCOM names where a real position would move the price against itself. See TODO `P11` (⚠️ this line cited `P7` until 2026-08-22 and that was already the wrong item).
...
3. ⚠️ **A live-scoring module (`P7`) has ~10 more sessions of usable FEATURES than the old
...
because a 7-name cross-section is not a cross-section. ✅ **FIXED 2026-08-23** — the universe re-scrape put **771 names** back on the last session (CLAUDE.md §6-2-bis), so the live-scoring module (TODO **`P7`**) is no longer blocked.
...
ranking. It is a small module — but it does not exist. TODO **`P7`**.
...
strategy is tradable at any size. TODO **`P11`**.
...
cannot absorb a position, and the backtest models no ADV cap and no slippage (`P11`).
