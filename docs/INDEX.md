...
| [ISSUES.md](ISSUES.md) | **34.9k** | *what is BROKEN?* 71 open / 38 resolved, codes permanent. ⚠️ **Read before quoting any number — four entries change how a number may be READ** (`NUL-1`, `NUL-3`, `RPR-1`, `OUT-1`), `CFB-1` before quoting a BID cash flow, and `TPL-1`/`CRP-1` before any non-bank financials parse |
