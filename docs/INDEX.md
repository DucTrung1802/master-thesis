...
>
> ⚠️ **NEVER BULK-LOAD THIS CORPUS.** 126 `.md` files, **~509k tokens** — about 2.5× a
> full context window. The routing below is the whole point: **open ONE file, when you
...
| [ISSUES.md](ISSUES.md) | **16.5k** | *what is BROKEN?* 15 open / 37 resolved, codes permanent, never renumbered or reused. ⚠️ **Read before quoting any number — four entries change how a number may be READ** (`NUL-1`, `NUL-3`, `RPR-1`, `OUT-1`) |
| [TODO.md](TODO.md) | **44.9k** ⚠️ | *what is NEXT?* ⚠️ **RENUMBERED 2026-08-23** — the re-scrape and the gold carry-up closed, so every code moved **down by 2** and today's `P1` is `STA-1`. **37 open tasks** in six lettered groups — **A data `P1`-`P3`, B OCR→Kaggle `P4`-`P7`**, C output `P8`-`P9`, D model `P10`-`P18`, E honesty `P19`-`P22`, F backlog `P23`-`P37`. ⚠️ A HYPHENATED code is RETIRED — **and that renumbering was BARE → BARE**, so a `P<n>` written before then means a different item; the 2026-08-22 crosswalk resolves it. **The largest file here: read the top, not the whole thing** |
...
claims, and locality is what keeps them true. **~227k tokens total — never open more than
one or two.**
