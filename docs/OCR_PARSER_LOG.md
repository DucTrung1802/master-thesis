# docs/OCR_PARSER_LOG.md — the filings/OCR chronicle

> ⚠️ **THIS IS `CLAUDE.md` §6-2-septies … §6-2-undeseptuagies, MOVED HERE ON 2026-09-06 AND NOT
> EDITED.** Every section heading, number, table and ⚠️ is verbatim what stood in the hub. Nothing
> was summarised, condensed or dropped in the move — only relocated.
>
> ⚠️ **EVERY `§6-2-*` CITATION IN THE REPO RESOLVES HERE.** There are ~196 of them
> (`docs/ISSUES.md` 74, `docs/TODO.md` 33, `src/kaggle_gpu/kgpu/PDF_OCR.md` 19,
> `src/web_scraper/CONTEXT.md` 23, `src/pipeline/CONTEXT.md` 1, `CLAUDE.md` 46), all of them
> **by section NAME rather than by anchor**, which is why the move costs nothing: the names are
> unchanged. A citation reading *"§6-2-quaterdecies"* means this file unless the section number
> is below `6-2-septies`, which stayed in the hub.
>
> **Why it moved.** `CLAUDE.md` is auto-loaded in full, in every session, whatever the session is
> about. This chronicle was **465,178 characters — ~116k tokens, 70 % of the hub** — and it is
> needed only when you touch `src/web_scraper/`'s filing parser. The hub's own opening paragraph
> says the corpus "can never be bulk-loaded" and that the index is "deliberately routing and not
> content"; the hub had stopped obeying its own rule. This file restores it.
>
> **What stayed in `CLAUDE.md`:** the current coverage state, the standing hazards
> (`BND-1`, `CRP-1`, `SET-2`, `TPX-1`, §5 rule 24), and a pointer to here. **Read the hub first.**
>
> ⚠️ **The register conventions are the hub's and hold here too** — a `⚠️` marks a claim that cost
> something to learn, no strikethrough, dates on findings, and a number without a null is
> descriptive rather than evidence.
