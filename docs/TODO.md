...
| **P40** | ⭐ **PARSE EVERY BID CASH FLOW FROM 2012 — 8 cells left, and every one of them is the CASH FLOW** | ~1 day GPU *est.* | ✅ | — | ⚠️ **Scope is FROM 2012 by decision** — BID files 12 documents a year only from 2012. ✅ **Q1-2012 recovered 2026-08-27** (`tail_continuation` + `label_wrap`); ✅ **Q3-2015 recovered the same day with NO new code** — `PGB-1`'s `_anchor` seam split had already made it parseable. **BID is now 163 / 171 = 95.3 %**, balance sheet **57/57** and income statement **57/57** COMPLETE. ⚠️ **`P39` IS NOW BLOCKED ON A MEASURED DEFECT, NOT MERELY UNMEASURED**: on Q3-2015 its FX recovery writes **MHB merger cash into the FX column**, and the identity CANNOT reject it because that row is the genuine fourth term. ⚠️ **AND A `periods` RUN DOWNGRADED Q1-2015**, the quarter included only to supply history — `PGB-1`'s remedy is unsafe for those. **↓ detail block**; CLAUDE.md §6-2-vicies |
...
| **cash flow** | **49 / 57** | **8** | ⚠️ **the entire remaining problem, in one statement**. Was 47; Q1-2012 and Q3-2015 both recovered 2026-08-27 |
| total | **163 / 171 = 95.3 %** | 8 | |
...
| `sane: magnitude 5.45e+08 vs typical 1.19e+14` | Q1-2026 | five orders out; no FX change touches it |
...
1. ⚠️ **FIX `P39` BEFORE RUNNING IT** — it is no longer merely unmeasured, it is measured WRONG
   ...
   item, not this one.
   ⚠️ **AND RUN EACH QUARTER ON ITS OWN.** A `periods` run may only WRITE the quarter it is
   repairing: the quarters added for history are re-parsed with a thinner history than they
   originally had, `sane` fails open on them, and an earlier and worse layer can win. Q1-2015
   was downgraded exactly that way and had to be restored from a backup — **take one first.** ⚠️ **Budget it on DOCUMENT SIZE**: the Q1-2012 run measured 9.1
