...
| ⭐ **P2** | ⚠️ **CARRY THE RE-SCRAPE UP TO GOLD** *(now the head of the list — `P1` closed 2026-08-23)* — `gold/stocks`, `gold/stocks_ta`, `gold/news_daily_panel`, then `filter/universe` and the `unified` partitions | ~2-4 h | ✅ | *old `P32`* | ⚠️ **THE HALF OF `P1` THAT IS NOT A SCRAPE, and `P1` LANDING ON 2026-08-23 MADE IT WORSE, NOT BETTER**: silver now runs to **2026-08-21** for 771 tickers while `gold.stocks` stopped **2026-07-08** and `gold.stocks_ta` **2026-06-26** — so the gap widened from 30/54 sessions to **~31/40 trading sessions against a FRESH silver**, and every `unified_schema_*` still reads gold. §5 rule 11 — *"re-scraped" never implies "re-ingested"* — with a number. ⚠️ **`filter_schema` and every `unified_schema_*` are downstream and re-materialise themselves never** |
