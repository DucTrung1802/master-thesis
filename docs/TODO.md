...
| **P40** | ⭐ **PARSE EVERY BID CASH FLOW FROM 2012 — 7 cells left, all `fx not mapped`** | ~1 day GPU *est.* | ✅ | — | ⚠️ **Scope is FROM 2012 by decision.** ✅ **Q1-2012, Q3-2015 and Q1-2026 all recovered 2026-08-27.** **BID is now 164 / 171 = 95.9 %**, balance sheet **57/57** and income statement **57/57** COMPLETE. ⚠️ **`P39` IS BLOCKED ON A MEASURED DEFECT**: on Q3-2015 its FX recovery writes MHB merger cash into the FX column and the identity CANNOT reject it. ⚠️ **AND EVERY `periods` RUN MUST BE DIFFED AGAINST A PRE-RUN BACKUP** — the history-supplying quarters get silently downgraded; measured twice now (Q1-2015, Q3-2025), both restored. **↓ detail block**; CLAUDE.md §6-2-vicies, §6-2-unvicies |
...
| **cash flow** | **50 / 57** | **7** | ⚠️ **the entire remaining problem, in one statement**. Was 47; Q1-2012, Q3-2015 and Q1-2026 all recovered 2026-08-27 |
| total | **164 / 171 = 95.9 %** | 7 | |
...
| | periods | waiting on |
|---|---|---|
| **`fx not mapped`** | **7** — Q4-2015, Q3-2016, Q4-2016, Q2-2017, Q3-2017, Q4-2017, Q3-2018 | ⚠️ **`P39` IS BLOCKED, and by a MEASURED defect rather than by not having been run.** Q3-2015 was one of these and it turned out to have **no FX line at all** — its fourth term is cash acquired in the MHB merger. `P39`'s recovery claims *the row between the two balances* as FX, so it wrote **1,477,340,000,000 of merger cash into the FX column**, and `_cash_flow_identity` cannot reject it because that row IS the genuine fourth term. **Fix the recovery before running it** — it must check the row's own label, or write to a column that says "unidentified fourth term". CLAUDE.md §6-2-vicies |
| `sane: magnitude 5.45e+08 vs typical 1.19e+14` — Q1-2026 | ✅ **DONE 2026-08-27** | ⚠️ **`sane` WAS RIGHT AND THE UNIT WAS WRONG.** Neither cash-flow page prints "Triệu VNĐ" while the balance sheet of the same filing does, so `unit_of` — which cannot tell *printed in đồng* from *did not say* — returned x1 and every figure was out by 10⁶, reconciling perfectly against itself. Fixed by `unit_from_document`, with the label repair ordered BEFORE it. CLAUDE.md §6-2-unvicies |
| `fx not mapped` — Q3-2015 | ✅ **DONE 2026-08-27** | ⚠️ **NO CODE WAS WRITTEN.** `PGB-1`'s `_anchor` seam split (2026-08-26) had already made it parseable — `onnx@200` passes both gates. ⚠️ **And it is the counterexample that blocks `P39`** |
| `no closing cash balance` — Q1-2012 | ✅ **DONE 2026-08-27** | ⚠️ **The diagnosis was wrong and the real cause was two defects, neither an OCR failure**: the statement's sparse LAST page was below `MIN_TABLE_WORDS` and thrown away, and the label wrapped AROUND its figures. ⚠️ **Fixing only the first produced a wrong figure that passed BOTH gates** — the opening balance written to the closing account. CLAUDE.md §6-2-noviesdecies |
