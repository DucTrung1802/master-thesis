...
| **P40** | ⭐ **PARSE EVERY BID CASH FLOW FROM 2012 — 9 cells left, and every one of them is the CASH FLOW** | ~1 day GPU *est.* | ✅ | — | ⚠️ **THE NEXT PRIORITY, set 2026-08-26; scope is FROM 2012 by decision.** BID files 12 documents a year only from 2012; before that an ANNUAL report and nothing else. ✅ **Q1-2012 RECOVERED 2026-08-27** — `tail_continuation` + `label_wrap`, 1h45m, and **exactly one period changed across all 9 CSVs**. **BID is now 162 / 171 = 94.7 % on that denominator**, with balance sheet **57/57** and income statement **57/57** COMPLETE. ⚠️ **EVERY REMAINING FAILURE IN THE TICKER IS A CASH FLOW.** ⭐ **`P39` is the next move and it is already written** — 8 of the 9 end at `cash flow unverifiable — fx not mapped`, so that one fix alone targets **170 / 171 = 99.4 %**. ⚠️ **DO NOT PASS `period_min: Q1-2012` TO THE RUN** — the floor is a DENOMINATOR; as a run parameter it deletes **9 real parsed cells** (`BRZ-1`). **↓ detail block**; CLAUDE.md §6-2-noviesdecies |
...
| **cash flow** | **48 / 57** | **9** | ⚠️ **the entire remaining problem, in one statement**. Was 47 until Q1-2012 was recovered on 2026-08-27 |
| total | **162 / 171 = 94.7 %** | 9 | |
...
| | periods | waiting on |
|---|---|---|
| **`fx not mapped`** | **8** | ⭐ **`P39` fix 1, already written** — the balances are recovered positionally and then discarded for want of a fourth term. **This one fix targets 170 / 171 = 99.4 %** |
...
1. ⭐ **Run `P39`** — written, unmeasured, claims 8 of the 9. Its ACB + VCB regression is part of
   that item, not this one. ⚠️ **Budget it on DOCUMENT SIZE**: the Q1-2012 run measured 9.1
