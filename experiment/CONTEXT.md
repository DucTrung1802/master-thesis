...
Single-document summary of **all methods and all results** across the five
experiments:
...
8. **experiments 4 & 5 build the first two data pieces:** a point-in-time **disclosure
   calendar** (exp_4) so fundamentals align to when they became public, and a point-in-time
   **shares-outstanding series** (exp_5) so raw price → market cap / turnover / free-float
   are computable without look-ahead.
