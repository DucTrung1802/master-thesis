...
| `unified_schema_vcb.return_5day__final__d1_h5` | 2 | 2 | 22 | 4,235 | 26 |
| `unified_schema_bank.cs_rank_5day__final__d20_h5` | 1 | 1 | 10 | 53,921 | 14 |
...
## 3. The name — `<target>__final__d<lookback>_h<horizon>`

⚠️ **The setup is IN the name, not only in the grouping, and that is forced.** Two of
the three groups share a schema AND a target and differ only in `lookback_d`
(`return_5day` at `d=20` and at `d=1`). A bare `return_5day__final` would have to hold
both or silently lose one. If two groups ever collide on a name anyway,
`plan_from_reports` **raises** rather than picking a winner.
...
⚠️ **203 of the 203 features in the biggest table come from runs that computed NO
NULL**, and no run in the archive is a clean pass (`feature_selection/CONTEXT.md`
§14b). A row in one of these tables is a channel some run ranked highly. **That is all
...
⚠️ **And the selections do not agree with each other.** 221 of 225 channels were
chosen by exactly one run, while 19 runs saw the same 27 `pool__basic` channels
