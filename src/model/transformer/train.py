# src\model\transformer	rain.py
"""Config -> run folder -> trained Transformer encoder -> scored result.

    python -m model.transformer
    python -m model.transformer --config transformer__all__rank_10day__final__d20_h10.yaml
    python -m model.transformer --config <path> --dry-run
...
the registry row. A TRANSFORMER run and an LSTM run therefore land in `index.csv`
...
DEFAULT_CONFIG = os.path.join(
    CONFIG_DIR, "transformer__all__rank_10day__final__d20_h10.yaml"
)
...
def train(config: Dict, runs_dir: str = RUNS_DIR, dry_run: bool = False):
    """Train one TRANSFORMER config end to end and return `(run_dir, metrics_table)`."""
    return engine.train(
        config,
        model_module=transformer_model,
