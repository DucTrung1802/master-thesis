# src\model\tcn	rain.py
"""Config -> run folder -> trained Temporal Convolutional Network (dilated causal) -> scored result.

    python -m model.tcn
    python -m model.tcn --config tcn__all__rank_10day__final__d20_h10.yaml
    python -m model.tcn --config <path> --dry-run
...
from model.tcn import model as tcn_model
...
def train(config: Dict, runs_dir: str = RUNS_DIR, dry_run: bool = False):
    """Train one TCN config end to end and return `(run_dir, metrics_table)`."""
    return engine.train(
        config,
        model_module=tcn_model,
