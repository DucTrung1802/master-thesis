# src\model\mlp\train.py
"""Config → run folder → trained MLP → scored result.

    python -m model.mlp --config mlp__vcb__return_5day__final__d20_h5__basic.yaml
    python -m model.mlp --config <path> --dry-run
...
from model.mlp import model as mlp_model

_HERE = os.path.dirname(os.path.abspath(__file__))

CONFIG_DIR = os.path.join(_HERE, "configs")
DEFAULT_CONFIG = os.path.join(CONFIG_DIR, "mlp__vcb__return_5day__final__d20_h5__basic.yaml")
...
def train(config: Dict, runs_dir: str = RUNS_DIR, dry_run: bool = False):
    """Train one MLP config end to end and return `(run_dir, metrics_table)`."""
    return engine.train(
        config,
        model_module=mlp_model,
