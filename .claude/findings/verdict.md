...
| **binary EVENT, VCB (2026-09-17)** | **`up_5pct_5day`** = `close[t+5] >= 1.05 x close[t]`, 11 feature groups selected on train only, 3 tables, 15-model grid | **36 model runs; the val-chosen model's test ROC-AUC is 0.520 / 0.549 / 0.564 against block-shuffled null p95s of 0.646-0.661; 1 run of 36 above its own bar** — val AUC ~0.70 falls to ~0.55 on 2024-2026. ⚠️ **The d = 1 setup (10 more runs, then a BANK-panel member) lifts the val-chosen ensemble to 0.660 vs p95 0.644 and then 0.670 vs 0.639 (refit 0.680) — null max 0.785, z +1.93, 2024 at 0.33, a second read of the test years: above the bar, not a pass** | 📂 [single-stock.md](single-stock.md) §6-1-quinquies |
