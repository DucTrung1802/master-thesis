...
|---|---|---|
| [ISSUES.md](ISSUES.md) | **105.1k** | *what is BROKEN?* 231 open / 55 resolved, codes permanent. ⚠️ **Read before quoting any number — four entries change how a number may be READ** (`NUL-1`, `NUL-3`, `RPR-1`, `OUT-1`), `CFB-1` before quoting a BID cash flow, `CFV-1` before believing any cash flow accepted at a STRICT layer, and `TPL-1`/`CRP-1` before any non-bank financials parse |
...
| [data-state.md](../findings/data-state.md) | **41.5k** | asking what the DATA looks like today. §6-2-§6-3 — the `pool__ta` prune, the 2026-08-23 re-scrape and carry-up, the freshness distribution, `DEP-1`, the filings/OCR summary, **MSN 2026-09-07 (the gaps were REFUSALS: `JVW-2` and `MSO-5` fixed, `PBT-1`/`SPN-2`/`BSP-1` open)**, the audit |
...
| `.claude/tools/replay_operating_profit.py` | ⚠️ **THE TOOL THAT RULED OUT THE SECOND-LARGEST REFUSAL BUCKET AS A ONE-LINE FIX, AND THEN FOUND THE REAL ONE.** Replays `_operating_profit_identity`'s residual over every income statement whose EVERY deepest-layer reason is `operating profit does not close` — read-only, no OCR. **Measured 2026-09-13 over VN30: 19 of 24 residuals ARE one row the reading produced, and 17 of those rows are ONE account** (the associates/JV share), which is `JVW-2`. ⚠️ **Its first answer was `0 of 24` and was a TYPO** — `absent_rows` stores `{key, label, values}` dicts where the accepted side's `row_dump` stores lists — so re-read the field names before trusting a new run of it. Re-run it after any parser change to this identity |
| `.claude/tools/audit_winnable_ceiling.py` | ⚠️ **THE DENOMINATOR A COVERAGE GOAL NEEDS, AND IT IS NOT THE GRID.** Sorts every open cell of a universe into PERMANENT (the correct answer is `missing`, §5 rule 24) or WINNABLE, by asking the filing itself — page tree and text layer, nothing rendered, no OCR, no GPU. **Measured 2026-09-13 over VN30 AFTER `CEI-1`: of 436 open cells **69** are permanent** — 43 on a long text-rich filing where no filing of the PERIOD carries the statement, 16 `OPB-2`, 10 `BND-3` — so the ceiling is **5,145** and 4,778 reads 91.6 % of the grid / **92.9 % of what is winnable**. ⚠️ **Its earlier answer of 28 permanent counted a SCAN WITH A TITLE PAGE as a text layer** (3-8 chars/page) and never asked whether another filing of the period had the statement; the same-period check first answered `100 % winnable` because it globbed the whole ticker folder. The superseded reading was: of 477 open cells only 28 are permanent — 16 `OPB-2` (a de-cumulation operand the issuer never filed) and 12 `BND-3` (a ≤10-page filing with a text layer and no wording) — so the honest ceiling is **5,186 of 5,214** and 4,737 parsed is **90.9 % of the grid, 91.3 % of what is winnable**, needing **+216 cells for 95 %**. ⚠️ **THIS CORRECTS A MORE GENEROUS FIGURE**: `~5,155 / 90.7 % of winnable` was quoted before and the filings justify only 28 permanent cells, so the target is HARDER than that claim implied. ⚠️ **A SCAN IS NEVER CALLED PERMANENT** (`SET-2`, `BND-3`): finding the wording proves presence, not finding it proves nothing without a text layer — **200 of the 477 are scans**, the genuinely unknown block. `TRC-1`'s truncated downloads count as WINNABLE because a re-scrape fixes them. ⚠️ **Its first run printed no rate at all**: `coverage()`'s `cells` is the GRID TOTAL and `open_cells` is the remainder, and it read `cells` as the parsed count. | 
| `.claude/tools/audit_band_ceiling.py` | ⚠️ **THE ONLY TOOL THAT SAYS WHETHER A CONTIGUITY GOAL IS REACHABLE AT ALL — `BND-3`.** For every hole on a ticker within reach of one unbroken band, it asks whether the document PHYSICALLY CONTAINS the statement the hole needs (page tree + text layer, nothing rendered). **FPT Q1-2009 and Q3-2010 are two-page filings with a text layer and no cash-flow wording, so FPT can never be a single band** (§5 rule 24). ⚠️ **The test is asymmetric**: finding the wording proves presence, not finding it proves nothing on a SCAN, so a scan reports `unknown` and never `absent` — `SET-2`'s mistake one level up. Also reports `TRC-1`'s truncated 0-page filings |
| `.claude/tools/audit_cash_close_blank.py` | ⚠️ **`CCB-1` AND `HOL-1` — TWO KINDS OF CELL THAT COUNT AS PARSED AND HAND THE READER ALMOST NOTHING.** `hollow_rows()` adds the second: **64 of 1,667 `pdf` income statements hold ≤6 figures against a median of 19**, split Q1 0.2 % · Q3 0.5 % · **Q2 5.0 % · Q4 9.2 %** — the CUMULATIVE quarters, 59 of 64 at `months=3`, balance sheet 0 of 1,679 — so `_decumulate`'s column-wise subtraction propagates a sparse prior forward. ⚠️ **Both of this file's measurements were wrong on the first try** (metadata counted as content; `absent_rows` read as lists): check the field names before trusting a new run. **`CCB-1` — A `pdf` CASH-FLOW ROW THAT COUNTS AS PARSED AND HANDS THE READER NULL.** Reads the statement CSVs, no OCR: rows whose `source='pdf'` while every closing-cash column is blank. **113 of 1,702 = 6.6 %** (2026-09-13), BVH 41 · FPT 13 · VIC 13 · BSR 12 · PLX 10. `reconcile` can satisfy its closing-balance requirement on a term that fills no schema column, which is exactly what `GCW-1` and `replay_cash_close.py` argue about — **predicted there, measured here.** Run it beside any coverage number off these files: the cell is not `missing`, it is present and hollow |
| `.claude/tools/replay_jvw2.py` | ⚠️ **`JVW-2`'s VALIDATION, AND THE TOOL THAT CAUGHT ITS FIRST REACH CLAIM BEING WRONG.** Replays the stored rows of every `operating profit does not close` cell through `map_to_schema` and the identity — no GPU. **A score over `SCHEMA_MATCH` is not a mapped column** (§5 rule 21): on the DEFAULT path 23 of 24 do not map, because ⚠️ **`ACCOUNT_WORDING` is consulted ONLY under `equity_wording`** — layers 66-69 of 115 — and replayed THERE **18 of 24 map and the identity CLOSES**. It asks both paths, because the difference between them IS the finding |
| `.claude/tools/replay_text_layer.py` | ⚠️ **TODAY'S PARSER ON A TEXT-LAYER FILING, WITH NO GPU — THE ONE REPLAY THAT CAN VERIFY A DETECTOR.** A run folder stores rows, not word boxes, so no stored-row replay can say whether a column detector fires; a TEXT-LAYER statement page is read from the PDF's own words, so with OCR switched off this drives `_parse_cascaded` itself over any layer subset (`--flag`) or every open cell of a ticker (`--open`) — CPU, no lock, no writes. **Built for `MSC-1` 2026-09-13, where it found four shapes `absent_rows` could not show.** ⚠️ Its null is a monkeypatch in a second process; ⚠️ `sane` has no band here, so a WON is a candidate for a run, never a banked cell (TPB Q1-2010 won under fix AND null: a screen-held cell) |
...
| `.claude/tools/survey_ocr_value.py` | ⚠️ **WHETHER FINISHING THE OCR PAYS FOR THE 5-SESSION BASKET — MEASURED 2026-09-17: NOT FIRST.** Read-only, no GPU, ~3 min. On the VN30 panel it measures the corpus's REACH in the basket universe (34 of 228 LIQUID names, 10.6 % of name-sessions), the event rate after a filing is published (+2.1 pp, z +1.70 on non-overlapping sessions), the within-session IC of parsed growth figures against a **ticker-permutation null** (profit growth FAILS; revenue growth +0.062, z +3.26, against +0.094/−0.133 for free price channels) and the paired out-of-sample increment of adding them to an event logit (daily AUC **−0.005 [−0.010, −0.0003]**). ⚠️ A within-session shuffle is too narrow a null for a figure constant for ~60 sessions — the tool prints both so the difference stays visible. Re-run it when OCR coverage grows. [data-state.md](../findings/data-state.md) has the tables |
| `.claude/tools/measure_auc_split.py` | ⚠️ **WHICH AUC IS THE BASKET'S METRIC, AND HOW MUCH OF ITS EDGE IS TIMING.** Read-only over `trials.csv` and the chosen row's session files — no refit, no GPU. **Measured 2026-09-18**: the session's own score level predicts that session's base rate at Spearman **+0.250** (z +6.43 vs 200 session permutations), and at `min_prob` 0.36 the traded sessions carry base 0.166 against 0.134 overall — **16 % of the edge is choosing the SESSION, 84 % choosing the NAME**, so a basket with a cut DOES trade timing and the earlier claim that it cannot was wrong. ⚠️ Its model-selection half is **not** evidence: n=13 CORRELATED models give `SE(Spearman) ≈ 0.29`, wider than every gap it prints. Re-run it after any change to `BASKET_CHOOSE_ON` or the cut | 
...
| [event_chain.md](../context/event_chain.md) | 10.1k | ⚠️ **THE BASKET, and nothing else since 2026-09-18** (`--setup basket`, §7): which ≤ X of LIQUID's 228 names to buy at the OPEN of the next session so they are +g % h sessions later — the label `upopen_5pct_5day`, the P(event) cut that lets a basket hold fewer names or none, and `event_chain.basket --pick <date>`. §6 is what the three DELETED close-to-close setups (VCB window/tabular, MBB) proved and how to recover them — *does VCB rise ≥ g % within h sessions?* — its parameters (`src/utils/event_target.py`), the new `pool__event_features`, the leakage guards, and ⚠️ **§5 THE TRIAL LOG** (`reports/event_chain/trials.csv`) — one row per model run of a COMPLETE trial: data, features, target, split, model, hyperparameters, run time, GPU, results |
...
| `../reports/event_chain/**` | per trial | ~1k each | **generated by `event_chain.report` / `event_chain.trial`** — one `report.md` per table and a copy inside every `trials/<id>/`. ⚠️ Read `trials.csv` first: every model run a number was chosen from, one row each |
