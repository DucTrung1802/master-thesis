...
> measurement in CLAUDE.md or a `.claude/context/` file and is **deleted, not ticked**.
>
...
> behind is the point and it is cited BY CODE from CLAUDE.md and the `.claude/context/` files.
>
...
>
> ⚠️ **AND `P54` / `P55` WERE EACH WRITTEN TWICE ON 2026-09-04 — A COLLISION, WHICH IS THE ONE
> THING A PERMANENT NAME CANNOT SURVIVE.** Two rows sharing a code makes BOTH uncitable, so it is
> not the same as renumbering and it had to be resolved. The EARLIER row of each pair keeps its
> code, because CLAUDE.md already cites both (`P54` at §6-2-quinquinquagies, `P55` at
> §6-2-sexquinquagies); the newer one took the next free number:
>
> | written 2026-09-04 as | is now | what it is |
> |---|---|---|
> | `P54` (second) | **`P60`** | `SEC-1` — eight VIC balance sheets on disk that do not add up |
> | `P55` (second) | **`P61`** | FPT's twelve income statements refused on magnitude — ✅ DONE the same day |
>
> ⚠️ **A `P54` or `P55` written on 2026-09-04 may mean either row**; take the subject, not the
> number. Nothing else in the file moved, and no code was reused.
>
> | group | items | what it is |
> |---|---|---|
> | **0 · PARSER** | `P54`, `P55`, `P60`, `P62`, `P43`, `P44`, `P45`, `P46`, `P48`, `P51`, `P52`, `P53` (`P39`/`P41`/`P42`/`P47`(b)/`P49`/`P50`/`P61` done) | ⚠️ **ADDED 2026-08-27, ABOVE EVERYTHING** — a review of `cafef_pdf_parser.py` + `cafef_financials.py` against the parsed tickers on disk. Two classes are WRONG NUMBERS every gate passes; two are the cost `P38`/`P6` are budgeted on |
...
| **P67** | ⭐ ⚠️ **THE 9-MONTH OPERAND IS ON DISK IN THE ARTEFACTS, AND THE LADDER WAS DROPPED ON A MEASUREMENT THAT ASKED THE CSV** | ~2-3 days | ✅ CPU, but it is a PARSER change | ⚠️ **`_quarter_priors` DEMANDS EVERY Q1..Q(q−1) AS A THREE-MONTH `pdf` ROW**, so a Q4 income statement needs three operands and dies on any one of them. An accountant subtracts ONCE: **Q4 = FY − 9M**, where the 9-month figure is the Q3 filing's own cumulative column, printed on its face. `cumulative_ladder.py` measured `months` on the stored ROWS, found nothing carrying a 6- or 9-month span, and the ladder was dropped — correctly, for the question it asked. ⚠️ **BUT THE PARSER WRITES ONE COLUMN AND DUMPS ALL OF THEM**: `accepted.income_statement.row_dump` carries every numeric column the page printed, which is how `replay_duplicate_column.py` works at all. Measured 2026-09-11 over the **21 blocked Q4 income statements**: a plausible 9-month column is **already on disk for 6** — GAS Q4-2022 (median ×2.92 over 21 rows), MBB Q4-2017 (×4.03), MCH Q4-2016 (×2.85), SHB Q4-2012 (×2.39), SHB Q4-2019 (×5.78), SSI Q4-2020 (×3.35) — and for the other **14 no run folder holds an accepted Q3 income statement**, so the operand needs ONE document re-parsed each, not three cells won. GVR Q4-2023's second column is a prior-year comparative (×0.99 on 30/68 rows) and is the shape the guard has to reject. ⚠️ **THIS IS THE LARGEST REMAINING LEVER AND IT NEEDS NO GPU**: 21 of the 37 cells between the corpus and 95 %. ⚠️ **AND IT IS THE MOST DANGEROUS ONE** — a column that is not the 9-month is a wrong figure in a column of billions, so the change is `_decumulate` plus a guard that proves the column from the filing's own arithmetic, never a heuristic ratio. ⚠️ **AND THE SHAPE OF THE WORK WAS READ WRONG ON THE DAY IT WAS FILED, SO CORRECT IT BEFORE ESTIMATING AGAIN.** This looked like a `pdf_ocr_merge._quarter_priors` change — add a one-operand path beside the three-operand one. It is not. The blocked branch is `_decumulate`'s `priors is None` skip, and feeding it needs the **9-month COLUMN**, which nothing on disk identifies: `row_dump` carries every numeric column the page printed and says only which one `quarter_column` picked. ⚠️ **A RATIO CANNOT IDENTIFY IT** — the six candidates run ×2.39 to ×5.78 and GVR Q4-2023's prior-year comparative runs ×0.99, so any band wide enough to admit SHB Q4-2019 admits a wrong column. The only thing on the page that says which column is the nine months is its **HEADER**, so the work is `cafef_pdf_parser` recording a `cumulative_column` beside `quarter_column` — the same kind of change as `income_by_columns` — and only then the merge path. `ninemonth_probe.py` is the measurement; re-run it before building. |
| **P66** | ⚠️ **THE PREMISE OF THIS ROW EXPIRED 2026-09-11 — THE GPU IS SPENT AND THE CORPUS IS 94.1 %, NOT 75.5 %.** Exactly ONE document in the corpus holds a cell no full 115-layer cascade has met (ACB 2009-Q3, a three-page Mẫu CBTT-03 summary form); MSN's ten documents cost 2.51 h for 0 cells. **What is left is a NEW READING, not more GPU** — see `P67`. The original row follows because its classification of the 765 is what the last four days were planned on. ⭐ ⚠️ **THE CORPUS IS 75.5 % PARSED AND THE TARGET IS >95 % — WHAT IS LEFT IS 299 DOCUMENTS OF GPU, 9.5-27.9 h, NOT A PARSER FIX** | ~1-2 days GPU | ❌ T4 or local | ⚠️ **THE CEILING WAS NEVER THE PARSER.** Of the 765 cells that had a filing and no `pdf` row on 2026-09-10: **332 had never been opened by any run**, **214 met only a cascade SHORTER than today's 117 layers**, **216 had PARSED and were never written**, and **3 lost to today's cascade**. ✅ The 216 were recovered the same day with no GPU — `merge_batch` runs the arithmetic screens itself now and withholds only what fails the filing's own identities (177 released, 51 held) — taking the corpus to **3,370 / 4,464 = 75.5 %** and SHB from 39.0 % to 72.4 %. ⚠️ **THE REST IS 589 CELLS IN 299 DOCUMENTS**: SSI 46 · GAS 43 · STB 39 · SHB 34 · SAB 22 · TCX 16 · VIC 13 · MCH/VHM/GVR 12 each, at 1.9-5.6 min a document. Run them with `QUARTERS = []` and `OVERWRITE = False`, which opens exactly those and nothing else. ⚠️ **GAS's 43 ARE NOT A DEFECT** — that ticker was last run on a 100-layer cascade ending at `cashbs`, before `GCW-1`'s `cash_wording` shipped the same day, and 28 of its refused cash flows print the one label that alias was written for. The original reading follows. **3,193 of 4,464 grid cells read `pdf` = 71.5 %**, of which **465 have no filing behind them** (`missing` is correct and permanent, §5 rule 24), **41 are SETTLED**, and **765 are WINNABLE**. ⚠️ **ONLY THREE TICKERS CLEAR 95 %** — CTG and VCB at 100 %, FPT at 98.6 % — and the winnable cells are concentrated: **SSI 136 · SHB 122 · STB 114 · GAS 59 · SAB 50 · TCX 48 · VHM 44 · MCH 42 · MBB 34**, which is 649 of the 765 in nine tickers. ⚠️ **THE GPU IS NOT THE CONSTRAINT — THE PARSER IS.** Of every run since 2026-09-07: **2,602 of 2,994 statements accepted (86.9 %)**, **688 of 998 quarters complete (68.9 %)**, and **49.5 % of 74.7 h went to documents that never produced all three** (1.9 min median for a complete one against 5.6 for a two-of-three, because `_parse_cascaded` breaks only when all three are in). So raising the rate is the ONLY lever that makes a run cheaper and better at once. ⚠️ **AND THE ORDER IS MEASURED RATHER THAN CHOSEN**: 236 of the 428 absent statements give ONE reason from EVERY layer — `no such statement on any page` **48** (`_page_kind`; `SET-2` says six of eight FPT cells recorded that way were winnable after a classifier change) · `is: operating profit does not close` **44** · `bs: assets != liab+equity` **30** · `cash: no closing balance` **26** · `fragmented reading` **26** (`SPL-1`) · `is: no profit before tax` **18**. Work that list down, top first. ⚠️ **THE ALTERNATE FILING IS NOT THE ANSWER TO THE REST** — 240 of the 310 short documents have no alternate at all, so `ALT-1` caps out near +48. ⚠️ **AND A DEFAULT-PATH REPAIR NEEDS `NST-1`'s STANDARD**: re-map every archived statement before shipping it, because a widened gate writes wrong figures past both gates (`PYR-1`, `SLD-1`, `EQW-1`). `pdf_ocr_batch.parse_scorecard` and `refusal_histogram` recompute all of it from the artefacts — free, no OCR — and the control notebook's §7, §8 and §10 print it. `RAT-1` |
| **P63** ✅ | **DONE 2026-09-05 by `CBS-1`** — ⚠️ **`CFV-1` — A CASH FLOW ACCEPTED AT A STRICT LAYER IS CHECKED BY NOTHING, AND TWO WRONG CLOSING BALANCES GOT IN** | ~3 h | ✅ CPU | `_cash_flow_identity` — `opening + movement + fx == closing` — rides with `relax_totals`, so a statement accepted at a STRICT layer never meets it. That has always been true and always been recorded; what 2026-09-05 measured is what it costs once `CLN-1` made a seal-damaged closing balance PARSE. **FPT Q3-2025 reads `09.853151273133` (9,853,151,273,133) against an identity of 9,853,512,731,370 and was WRITTEN** before an arithmetic screen over the CSV caught it and it was reverted from a pre-session copy; **FPT Q1-2025 reads `6.755.015,214:252` against 6,755,645,214,252** and was held back at the merge. ⚠️ **THE ASYMMETRY IS THE ARGUMENT**: on a BALANCE SHEET `reconcile` tests `assets == resources` on EVERY layer, so a damaged recovered total is caught; on a CASH FLOW nothing is. ✅ **The population is small and already measured** — 6 of the 7 FPT cash flows written that day close their identity to **EXACTLY 0**, and over the ticker's 48 checkable rows on disk exactly 1 fails. ⚠️ **MEASURE THE FALSE-REFUSAL RISK ON THE POPULATION THE GATE WOULD SEE** (`P49`'s standard, and its lesson that the statement CSVs are the WRONG population — they hold de-cumulated rows `reconcile` never sees): every ACCEPTED cash flow in every run folder on disk, answered / abstained / failed. A filing with an unmapped fourth term (a merger line, which is why `cash_extra_terms` exists) is the case that would start failing, and BID has three. ⚠️ **ABSTAIN on an unmapped term, exactly as `OP_IDENTITY` and `SECTION_SUMS` do** — a chart or a reading that cannot supply all four must not be refused for lacking them. `CFV-1` · §6-2-sexsexagies | ✅ **CLOSED 2026-09-05, and the gate was printed on another page of the same filing**: VAS ties B03's code 70 to B01's code 110, so `reconcile` now compares the closing balance against the BALANCE SHEET's cash line on every layer. `bank` is excluded on a measurement (0 of 226 quarters agree); on `corp` it answers 208 times, agrees 193, and every one of the 9 distinct disagreements is a genuine defect. §6-2-septsexagies
| **P64** ✅ | **DONE 2026-09-05 — six of the nine; the other three are PERMANENT or a slid tail** — ⚠️ **FPT's LAST NINE CELLS — three are one quarter blocking two, and the figures are already verified** | ~4 h | ✅ CPU | FPT is **204 of 213** and the nine that remain are refusals with measured reasons, not coverage. **2 are PERMANENT** (2009-Q1 and 2010-Q3 cash flows — the condensed disclosure form contains none, §5 rule 24). **1 is the gate working** (2012-Q1 cash flow: the reading puts the 1-Jan OPENING in the CLOSING slot, so `sane` sees it "exactly equal an already-accepted quarter"). **3 are the seal** (`P60` above, plus 2016-Q1's balance sheet — parsed with 70 correct line items and ONE total reading **24.695.452.363.505** against a printed **24.695.453.363.505**, 4e-8 relative and therefore inside `_equal`'s tolerance, held back at the merge; the four verified figures are in the per-ticker notebook). ⭐ **THE THREE WORTH THE WORK ARE 2015-Q1's INCOME STATEMENT AND THE TWO IT BLOCKS** (Q2 and Q4 print a cumulative P&L that cannot be de-cumulated while Q1 is `missing`). ⚠️ **The page is a LANDSCAPE rotated scan with FOUR columns — quarter this year / quarter last year / YTD this year / YTD last year — and FOR A Q1 COLUMNS 0 AND 2 ARE THE SAME QUANTITY.** Column 0 is misread in exactly two cells and column 2 has both right, read off the page: `9_chi_phi_ban_hang` **522.723.919.797** (col0 reads 522.721.917.777) and `7_doanh_thu_hoat_dong_tai_chinh` **89.420.051.565** (col0 reads 89.420.081.565); with those two, `11 = 5+7-8-9-10` closes to the đồng at 628.940.977.843. ⚠️ **No layer reads column 0 correctly** at 200, 300 or 400 dpi, with `+red`, `+joinlost` or `+deskew`. ⚠️ **A rule that prefers the cumulative column on a Q1 is defensible in principle — the two columns ARE the same quantity — and it needs a blast-radius measurement before it ships**: it changes which column `_first_value` reads, which is the DEFAULT path for every four-column income statement. `IBC-1`-style guards (only a Q1, only where the two disagree, only where one closes the identity and the other does not) are what would bound it |
| **P65** | ⭐ ⚠️ **VIC IS 46 OF THE 56 CELLS LEFT IN THE WHOLE CORPUS — measured 2026-09-05** | ~1 day | ❌ T4 | Every `statements/**/*.csv` against `documents(allow_parent=True)` and `settled_absences`: **130 missing cells across 8 tickers, and only 56 are winnable.** **66 are quarters the company never filed** (`missing` is correct, §5 rule 24) and **8 are SETTLED**. ⚠️ **THE `pdf` COUNT INVERTS THE RANKING**: BID looks second-worst at 182/210 and has **4** cells left; **CTG and VCB are 210/210**; **FPT is 210/213**. ⚠️ **VIC is the only parsed ticker with NO unfiled quarter at all** — all 47 of its missing cells have a filing on disk — so it is where the next OCR hour goes, and the other five tickers are **10 cells between them** (ACB 2, BID 4, BSR 2, FPT 1, TCB 1). ✅ **27 of VIC's 46 are INCOME STATEMENTS and most name the operand they wait for** — a cumulative Q2/Q4 cannot be de-cumulated while its own Q1/Q3 is `missing`, so they unblock in cascade as the balance sheets and cash flows land, and this is not 46 separate problems. ⚠️ **`corp`, so `CRP-1` stands and nothing from it may be quoted as a fundamental.** The full cell list, the three-way split and the re-measurement command are `.claude/docs/PDF_OCR.md` §8. ⚠️ **Re-run the split after any `_page_kind` change** — six of FPT's eight SETTLED cells turned out winnable after the 2026-09-04/05 classifier work (`SET-2`) |
...
| **P61** ✅ | **DONE 2026-09-04 — and the cause was NOT the unit. FPT 154 → 188 cells** | ~5 h GPU *actual* | ⚠️ GPU | ✅ **`GTR-1`: the GIẢI TRÌNH page.** Circular 155/2015 makes an issuer explain a profit swing over 10 %, and FPT prints that explanation on the page IMMEDIATELY AFTER its income statement — a five-column grid in **`ĐVT: Triệu đồng`** where the statement itself is in đồng. It carries a real table, so `_fill_continuations` absorbed it as page 2, **and its rows are the statement's OWN account names**: `Tổng lợi nhuận kế toán trước thuế` appears TWICE, and `Statement.find` — which skips a row with no value — returns the SECOND. ⚠️ **This item predicted the unit and said the unit was ruled out, and both halves were right**: the unit IS correct and something else answered `C_PBT`; the answer was in the run folder's `absent_rows` and cost no OCR, exactly as the item said. ✅ Fixed as `AUDIT_NS`' rule at the other end of the filing, on the branch that absorbs an UNIDENTIFIED page, so the worst it can do is end a run one page early. **All twelve recovered, and the twelve cumulative Q2/Q4 they blocked with them.** ⚠️ Four more defects fell out of the same job — `VAS-3`, `NOT-2`, `ROT-2`, `ROT-3` — see `P62`. CLAUDE.md §6-2-quinquasexagies | Every Q1 and Q3 from 2020 is refused by `sane` at ~1e6 against a typical 6.58e11 — Q3-2020 1.74e4, Q1-2024 2.53e6, Q3-2024 8.11e6, Q1-2026 2.80e6 — i.e. the probe reads as though the figures were printed in triệu đồng and never scaled. ⚠️ **Each one blocks a CUMULATIVE Q2/Q4 that then has no prior to subtract**, so twelve quarters are worth roughly twenty-four cells, which is most of FPT's remaining 38. ⚠️ **NOT a regression**: all twelve were `missing` before 2026-09-04 too; only the refusal reason changed. ⚠️ **AND THE OBVIOUS CAUSE IS RULED OUT** — `onnx@300+deskew` reads Q3-2024's net revenue correctly at 15,972,397,069,700 with `unit=1`, so the UNIT is right and something else answers the `C_PBT` anchor. **Probe which row `C_PBT` maps to at the winning layer before spending another cascade**; the answer is in the run folder's `row_dump` and costs no OCR. CLAUDE.md §6-2-quattuorsexagies |
| **P62** ✅ | **DONE 2026-09-05 — 16 of the 17 closed; FPT is 204 of 213 and the nine that remain are `P64`** | ~1 day GPU *actual* | ⚠️ GPU | ✅ **Nine defects, and not one an OCR failure** — `SEAL-1` (the round company seal stamped across a figure; the signature is that raising the resolution does not CONVERGE), `RHF-1`, `SPB-1`, `CDF-1`, `CDF-2`, `CDF-3`, `MSO-4`, `IBC-1`, `CLN-1`. ⚠️ **THIS ITEM'S OWN READING OF THREE OF THEM WAS WRONG, AND THAT IS WORTH KEEPING**: Q1-2012's income statement is NOT a `ROT-3` page — it lost its title AND its form code and was taken by the balance sheet above it (`IBC-1`), which also means the balance sheet had been written from pages [2, 3, 4]; Q1-2016's and Q3-2025's balance sheets are not one defect but three (a seal, a colon for a separator, and the `Mã số` heading read as `moso`); and **6 of the 8 cells this item called SETTLED were winnable** — `settled_absences` reads a verdict on `_page_kind` as a verdict on the DOCUMENT (`SET-2`), and `CDF-2`, `IBC-1` and `RHF-1` all change that function. ⚠️ **AND ONE FIX MADE A DAMAGED FIGURE PARSE**: `CLN-1` turned two seal-broken closing balances into readable WRONG ones, one of which was written and reverted the same hour — `CFV-1`, now `P63`. The merge diffed column by column: **17 cells changed, every one `missing` → `pdf`, 0 columns lost, not one existing `pdf` row altered.** CLAUDE.md §6-2-sexsexagies · `web_scraper` §3m. The original reading follows. Measured 2026-09-04 after `P61`: **188 of 213 cells `pdf`**, 17 open of which **8 are SETTLED** (the filing contains no such statement). **income statement 7** — ⚠️ five are cumulative Q2/Q4 blocked by a Q1/Q3 that itself fails, so **fixing Q1-2012, Q1-2015 and Q3-2010 closes six cells**. ⚠️ Each is a DIFFERENT defect: **Q1-2012**'s income statement is a turned page that `ROT-3` did NOT recover (its consolidated filing, page 4 — note a `glob` returns the PARENT filing, whose page 5 classifies cleanly, and reading that as a fact about the run is `ALT-1` with the entities swapped); **Q1-2015**'s is `operating profit does not close` behind 6-12 split figures at every DPI; **Q3-2010**'s filing is a **two-page CONDENSED summary** whose pages carry no statement title at all (`STT | Nội dung | Số dư cuối kỳ` and `STT | Chỉ tiêu | Kỳ báo cáo | Lũy kế`) — ACB's Mẫu CBTT-03 shape, §6-2-sesquadragies, and a rule again. **cash flow 8** — 3 `no closing cash balance`, 2 `cash flow does not close` where the closing balance is a literal TAIL of the right figure (Q1-2023 reads 424,451,239 against 3,289,424,451,229, which the SAME filing's balance-sheet cash line confirms), 3 fragmented. **balance sheet 2** — Q1-2016 and Q3-2025, both `assets != liabilities + equity`. ⚠️ **AND TWO ROWS ON DISK ARE WRONG WITHOUT BEING OPEN**: Q1-2025 and Q3-2025 income statements are `pdf` with `unit=1000000` applied to figures already in đồng (PAT **2,595,557,480,309,000,000**), and the 2026-09-04 re-read is wrong elsewhere in the same statement — **two wrong readings is not a repair**, so neither was forced. Q3-2025 propagates into Q4-2025, which reads **−2,663 tỷ**. ⚠️ **The free check that vouches for the rest and convicts these**: the four quarters of a year must sum to the audited annual, and FPT's do for **2011 and 2020-2024** against the published figure. CLAUDE.md §6-2-quinquasexagies |
...
| **P68** | ⚠️ **DID EARLY STOPPING COST THE BASKET ITS EDGE? — `event_boost_xgb_d8` with early stopping OFF on the `d = 10` dataset** *(opened 2026-09-19)* | ~3 min + a report | ✅ CUDA | The first `d = 10` trial is worse on every read (frozen basket **+0.24 % vs +1.98 %**, Sharpe@50 **−0.17 vs 0.65**) and its chosen `ensemble_xl` reads NO window, so `d` cannot be the cause ([event_chain.md](../context/event_chain.md) §9c). What moved for it: early stopping on val LOG-LOSS cut `event_boost` from 800 fixed rounds to **209 (best 159)**. Refit that one member at 800 rounds with `early_stopping_rounds = 0` on the same dataset hash, and score it and `ensemble_xl` beside the stopped pair. ⚠️ **One fit on one seed**: without a seed spread a small difference settles nothing, and `val_daily_auc` chose the round as well as the model (§9), so the TEST row is the only honest read. |
...
| `PRF-0/1/2/7/8/9` | **DONE**; cited by name from CLAUDE.md and `.claude/context/walkforward.md` |
...
⚠️ **AND A SECOND, INDEPENDENT INVARIANT SHIPPED ON 2026-09-05 AND COVERS THE `corp`
SIDE OF THIS ITEM: `CBS-1`.** VAS ties B03's code 70 to B01's code 110, so the same filing prints
the closing cash balance a second time, on its BALANCE SHEET, and `reconcile` now compares them on
every layer. It found 9 distinct wrong figures and would refuse 6 VIC disk rows. ⚠️ **It
does NOT reach the ten rows this item is about**: all ten are `bank`, where the balance sheet's cash
line is notes and coin against a far wider cash-flow aggregate — measured, the two agree on **0
of 226** bank quarters. The cross-quarter invariant below is still the only thing that can see them.
...
| study | pre | post |
|---|---|---|
| `.claude/context/model.md` §11 (h=5, foreign flow, 28 folds) | net@20bps **+1.46** (2017-20) | **−0.51** (2022-26) |
...
|---|---|---|
| **`PRF-1`** ✅ 08-19 | 10 expanding folds, 118 periods — **it is not one lucky split**. ⚠️ **My recorded prediction was HALF WRONG**: the level held, the decay did not (Sharpe@30 slope −0.100/fold) | `.claude/context/walkforward.md`; CLAUDE.md §6-0-a |
| **`PRF-8`** ✅ 08-19 | three architectures over identical folds, **101× of capacity, every \|t\| < 1** — the architecture is worth nothing at h=20 | `.claude/context/walkforward.md` §8; §6-0-ter |
...
| **`PRF-0`** ✅ 08-19 | the ceiling band does **not** bite the h=20 model; excluding it HELPS (+1.484 → +1.551) | `.claude/context/backtest.md` §8h |
...
| **`P0-1`** ✅ 08-17 | the two-layer null CLEARS. ⚠️ **My recorded prediction was WRONG** | `.claude/context/feature_selection.md` |
...
| **`P1-8`** ✅ 08-21 | `WFO-1` closed by a **REFUSAL** rather than a fix, and `RPR-1`'s half with it — ⚠️ it CREATED a limit (`P22`) | `.claude/context/walkforward.md` |
...
| **`P4-11`** ✅ 08-21 | layer-2 detection scoped to the chain being asked about — `pipeline` no longer calls another experiment's run `up to date` | `.claude/context/pipeline.md` |
