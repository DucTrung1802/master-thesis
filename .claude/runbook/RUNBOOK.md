...
| **G8** | the **BASKET**, the one event setup — which ≤ X names to buy at the OPEN of the next session so they rise ≥ g % in h sessions (`--setup basket`, universe LIQUID, X = `--top-k`, default 5, label `uphold_5pct_5day` — buy the OPEN of N+1, sell the CLOSE of N+5, 5 sessions held; `upopen` sells at N+6 and is carried beside it) | `dagster asset materialize -f src/orchestration/definitions.py --select "unified/pool__targets" --partition LIQUID`, then `python -m event_chain --setup basket --apply --notes "<why>"` — ⚠️ one stage per process (`--stages select`, `final`, `dataset`, `train`, `report`) on a 16 GB machine | targets 0.4 min · select **52 min** (2 pools, CUDA) · final + dataset 4 min · train **8 min** · report **21 min** (2026-09-19, after `WST-1` cut the design 906 -> 151 columns and the tree models moved to CUDA: the grid's whole fit fell 1,320 s -> 77 s) | → G9. ⚠️ the report is `event_chain.basket`: val-chosen on `config.BASKET_CHOOSE_ON` (**`val_daily_auc`, the WITHIN-SESSION AUC** — a pooled AUC also prices which sessions are eventful, which no basket trades), a random-basket null per session, ceiling names BOUGHT, and the val-chosen P(event) cut; the table is named `__d1_h5__` because the `hold` label spans 5 sessions (`upopen` spanned 6) ([event_chain.md](../context/event_chain.md) §7) |
