...
| **G8** | the **BASKET**, the one event setup — which ≤ X names to buy at the OPEN of the next session so they rise ≥ g % in h sessions (`--setup basket`, universe LIQUID, X = `--top-k`, default 5, label `upopen_5pct_5day`) | `dagster asset materialize -f src/orchestration/definitions.py --select "unified/pool__targets" --partition LIQUID`, then `python -m event_chain --setup basket --apply --notes "<why>"` — ⚠️ one stage per process (`--stages select`, `final`, `dataset`, `train`, `report`) on a 16 GB machine | targets 1.5 min · select 37 min (2 pools) · final + dataset 4 min · train + report 56 min (2026-09-18) | → G9. ⚠️ the report is `event_chain.basket`: val-chosen on `config.BASKET_CHOOSE_ON`, a random-basket null per session, ceiling names BOUGHT, and the val-chosen P(event) cut; the table is named `__d1_h6__` because the label SPANS 6 sessions ([event_chain.md](../context/event_chain.md) §7) |
| **G9** | **the basket of ONE session** — "what do I buy tomorrow morning?" | `python -m event_chain.basket --pick YYYY-MM-DD [--top-k 5] [--model <row>] [--min-prob 0.25] [--entry close|next_open]` | **~2 min** (three refits on ~707k rows, 2026-09-18) | G8 → prints at most X names with P(event) ≥ the val-chosen cut (cash when none clears it) and the realised outcome when N+6 is in the data; writes `reports/event_chain/<table>/picks/`. ⚠️ a date that is not a session resolves to the last session before it; the entry price is the NEXT session's open, which is not known when the pick is made |
