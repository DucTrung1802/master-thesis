...
| **G6** | give `model.event_panel` its PEERS — the BANK partition of the event pools | `dagster asset materialize -f src/orchestration/definitions.py --select "unified/pool__targets" --partition BANK`, then the same with `unified/pool__event_features` and `unified/pool__market_context` | 13 s · 12 s · 12 s (2026-09-17, 20 banks, 54,728 rows each) | G1 → G5. ⚠️ **after every G1 as well**: the panel member reads these at fit time and its peer rows are not in the dataset hash (`PEH-1`) |
| **G7** | the event chain on **MBB** — MBB's pools, then the tuned grid | `dagster asset materialize -f src/orchestration/definitions.py --select "unified/pool__targets" --partition MBB`, then the same with `unified/pool__event_features`, `unified/pool__market_context`, `unified/pool__market_breadth`, `unified/pool__news_daily`; then `python -m event_chain --setup tabular_mbb --apply --notes "<why>"` | 12-14 s per pool · select 5 pools ~8 min · train+report ~4 min (2026-09-17) | G6 → G4. ⚠️ `tabular_mbb` names its ticker; `--ticker` overrides it. The grid is `config.MBB_*` (`event_chain.md` §7), and the report also scores a monthly **rolling refit** |
