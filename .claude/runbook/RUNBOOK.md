...
| **G1** | put a NEW gain/horizon into the data | edit `src/utils/event_target.py`, then `dagster asset materialize -f src/orchestration/definitions.py --select "unified/pool__targets" --partition <T>` and `--select "gold/stocks_event_features,gold/market_context"` then `--select "unified/pool__event_features,unified/pool__market_context" --partition <T>` | 12 s · 14m 17s (the 2026-09-17 table, +27 channels) · 2 s · 2 s | — → G0. ⚠️ **all of them**, or the features describe a different event than the label. `gold/market_context` does not depend on the event and rebuilds in 2 s |
...
| **G5** | the TABULAR setup end to end — d=1, all channels of the pools that clear their null, linear kinds + fixed ensembles | `python -m event_chain --setup tabular --apply --notes "<why>"` | selection ~5 min per pool × 5; the rest ~2 min (2026-09-17) | G1 → ⚠️ the model grid and the ensembles are `config.TABULAR_*`, frozen from a CV that never read test; read the report's **refit on train+val** column beside the train-only test AUC |
| **G6** | ⚠️ **RETIRED 2026-09-18** — it built the BANK peer pools for `model.event_panel`, which was deleted with the single-ticker event setups ([model.md](../context/model.md) §19). The basket fits the LIQUID panel directly and needs no peer partition | — | — | — |
| **G7** | ⚠️ **RETIRED 2026-09-18** — the MBB event chain (`--setup tabular_mbb`) was deleted with the close-to-close label ([event_chain.md](../context/event_chain.md) §6: val 0.743, test 0.630 against a null p95 of 0.616, and the close of N is not a fill) | — | — | — |
| **G8** | the **BASKET**, the one event setup — which ≤ X names to buy at the OPEN of the next session so they rise ≥ g % in h sessions (`--setup basket`, universe LIQUID, X = `--top-k`, default 5, label `uphold_5pct_5day` — buy the OPEN of N+1, sell the CLOSE of N+5, 5 sessions held; `upopen` sells at N+6 and is carried beside it) | `dagster asset materialize -f src/orchestration/definitions.py --select "unified/pool__targets" --partition LIQUID`, then `python -m event_chain --setup basket --apply --notes "<why>"` — ⚠️ one stage per process (`--stages select`, `final`, `dataset`, `train`, `report`) on a 16 GB machine; ⚠️ **start a multi-hour stage DETACHED** (PowerShell `Start-Process`) — one started from a Claude session's background shell dies with the session, as the 2026-09-19 `train` did mid-forest — and **edit no ranking-path file while `select` runs** (`FPT-1`) | ⚠️ **THESE COSTS ARE `d = 1`'s AND EXPIRED ON 2026-09-19**, when the grid moved to `d = 10` for its LSTM/TCN members: the window tensor is 10x, `WST-1` short-circuits `d == 1` ONLY so the trees are back on a 906-column design, and the selection layer builds six statistics per channel again. **Measured on the first `d = 10` run (2026-09-19, after `MEM-2`): select 4 h 34 (2 pools) · final 1 min · dataset 3 min · train 35 min (10 models, three DL) · report 17 min.** What `d = 1` cost: targets 0.4 min · select **52 min** (2 pools, CUDA) · final + dataset 4 min · train **8 min** · report **21 min** (the grid's whole fit 1,320 s -> 77 s after `WST-1` + CUDA) | → G9. ⚠️ the report is `event_chain.basket`: val-chosen on `config.BASKET_CHOOSE_ON` (**`val_daily_auc`, the WITHIN-SESSION AUC** — a pooled AUC also prices which sessions are eventful, which no basket trades), a random-basket null per session, ceiling names BOUGHT, and the val-chosen P(event) cut; the table is named `__d10_h5__` since 2026-09-19 (`d` left 1 for the deep-learning members; it was `__d1_h5__`, and the `hold` label spans 5 sessions where `upopen` spanned 6). ⚠️ **The report RAISES when a declared model has no scored run** (`BRD-1`) — fit it or take it out of the grid; a board over a subset is not the search that was declared ([event_chain.md](../context/event_chain.md) §7) |
| **G9** | **the basket of ONE session** — "what do I buy tomorrow morning?" | `python -m event_chain.basket --pick YYYY-MM-DD [--top-k 5] [--model <row>] [--min-prob 0.30] [--entry close|next_open]` | **~2 min** (three refits on ~707k rows, 2026-09-18) | G8 → prints at most X names with P(event) ≥ the val-chosen cut (cash when none clears it) and the realised outcome when N+6 is in the data; writes `reports/event_chain/<table>/picks/`. ⚠️ a date that is not a session resolves to the last session before it; the entry price is the NEXT session's open, which is not known when the pick is made |
| **G10** | **re-choose the basket's P(event) cut** from a written basket report — no refit | `python -m event_chain.basket --min-prob-study [--top-k 5] [--entry close]` | **52 s** (2026-09-17) | G8 → writes `min_prob.{md,csv,json}` beside `report.md`; G9 reads the json. ⚠️ the cut is chosen on VAL (`config.BASKET_MIN_PROB_ON`); the test grid it prints is descriptive. `--entry close` re-prices the same picks at the close of N — the fill nobody gets (`EXE-1`) |
