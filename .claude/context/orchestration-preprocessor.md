...
| `_ingest_gold_stocks_event_features` (+27, 2026-09-17) | `har_`/`px_`/`flow_` in the same CTAS: RAW `high`/`low` for Parkinson (a split rescales both, the ratio is unchanged; `ln(H/L) > 0.5` is a corrupt print), `+1e-8` inside every `LN` of a variance, the foreign sums scaled in the `since` CTE because a window over a window needs its own SELECT |
| `_ingest_gold_market_context` (new) | one CTAS, PK `(date)`: indices from `gold.stock_market` (a close outside 0.8-1.2× its previous 5 sessions' range is NULL), US series from `gold.economy_usa` and yields from `gold.bonds` joined through a RUNNING `MAX(date)` over `(date, src)` so a same-date US row sorts AFTER the VN row — strictly before, `TZL-1` |
| `_ingest_unified_pool_market_context` (new) | `_helper_unified_pool_on_date_spine` |
