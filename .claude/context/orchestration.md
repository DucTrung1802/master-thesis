...
| `gold/stocks_event_features` (+27 channels, 2026-09-17) | `har_*` LOG volatility (Parkinson range at 1-125 sessions, close-to-close 5-125, log |r_1| and |r_h|, up/down semivariance), `px_*` range position / drawdown / MA gaps / 20-60 log returns, `flow_*` turnover ratio and foreign net value / foreign-room change scaled by their own trailing 250-session mean absolute | **14m 17s** all tickers (was 8m 50s); unified VCB pool 70 columns. VCB's `har_lpk_22` correlates 0.9996 with the research harness's own computation |
| `gold/market_context` (new) | ONE ROW PER VN SESSION: `mctx_vn_*`/`mctx_vn30_*` (log returns, log realised and Parkinson vol, range position, drawdown, MA gaps, turnover z, scaled foreign flow), `glb_*` (VIX, S&P 500, Dow, US 10y, broad dollar, WTI) and `bond_*` (VN 1/5/10y yield changes, 10y-1y slope). ⚠️ **US series and VN yields are LAGGED ONE VN SESSION** by a running as-of maximum (`TZL-1`); ⚠️ an index print 20 % outside its previous 5 sessions' range is dropped (VN30 carries ×16 prints); ⚠️ WTI's −37.63 on 2020-04-20 killed the first build (`LN` of a negative) — every log return is guarded | 1.8 s; 4,398 sessions × 59 channels. ⚠️ A correlated `LATERAL ... LIMIT 1` as-of join ran >10 min before it was cancelled |
| `unified/pool__market_context` (new) | the date-level table broadcast onto the spine (`DATE_SPINE_POOLS`, LEFT join) | 0.6 s on VCB, 59 channels |
