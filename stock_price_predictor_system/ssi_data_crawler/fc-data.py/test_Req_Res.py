...
def md_access_token():
    print(
        client.access_token(model.accessToken(config.consumerID, config.consumerSecret))
    )
...
def md_get_intraday_OHLC():
    print(
        client.intraday_ohlc(
            config,
            model.intraday_ohlc("fpt", "15/10/2020", "15/10/2020", 1, 100, True, 1),
        )
    )
...
def md_get_daily_index():
    print(
        client.daily_index(
            config,
            model.daily_index(
                "123", "VN100", "15/10/2020", "15/10/2020", 1, 100, "", ""
            ),
        )
    )
