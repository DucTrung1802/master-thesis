...
|---|---|---|---|
| **EVD-1** | substantive | Nothing upstream ever cleared a null. All 19 VCB source runs computed none and the bank run failed its own. **Not fixable downstream** — it needs selection runs that compute a null, which is GPU-hours. Everything below inherits it. | `feature_selection/CONTEXT.md` §14b |
...
| **DRF-1** | substantive | **Worse after the STL-1 rebuild**: 126 of 721 VCB channels put >1% of the test set beyond 5 train-sigmas and **18 put 100% of it there** (was 48 and 4 at 203 channels). Monotone macro levels. Reported in `drift.csv`, acted on by nothing. The fix belongs upstream — differenced channels, not a silent drop here. | `train_test_creator` → `drift.csv` |
| **COV-1** | substantive | `prop_buy_vol` was *selected* despite zero coverage across the whole train slice, on **both** the VCB and bank tables. The drop in `train_test_creator` is a workaround; the defect is that the selection ranked it highly on the 20% of history where it exists. | `train_test_creator/dataset.py::_screen` |
